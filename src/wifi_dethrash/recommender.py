...
class Recommender:
    ...
    def txpower_recommendations(
        ...
        for pair in confirmed:
            ...
            quieter = pair[1] if louder == pair[0] else pair[0]
            ...
            quieter_rssi = best.avg_rssi_b if louder == pair[0] else best.avg_rssi_a
            louder_txp = current_txpower_a if louder == pair[0] else current_txpower_b
            quieter_txp = current_txpower_b if louder == pair[0] else current_txpower_a
            ...
            if louder_rssi > self._rssi_floor:
                # Both signals healthy — reduce the louder/higher-txpower AP
                # Prefer higher txpower AP (more headroom)
                if louder_txp is not None and quieter_txp is not None:
                    if quieter_txp > louder_txp:
                        target_ap = quieter
                        reduce_txp = quieter_txp
                    else:
                        reduce_txp = louder_txp
                elif louder_txp is not None:
                    reduce_txp = louder_txp
                else:
                    reduce_txp = None

                if reduce_txp is not None:
                    suggested = max(reduce_txp - 2, 5)
            else:
                # Signals are weak — increase the quieter AP to improve
                # coverage and create differentiation
                action = "increase"
                target_ap = quieter
                if quieter_txp is not None:
                    suggested = quieter_txp + 2
                else:
                    skip_reason = "No txpower data to compute increase"
    ...
    def usteer_commands(
        ...
        """Generate usteer config: only signal_diff to prevent oscillation.

        SNR-based kicking (min_snr, min_connect_snr) is intentionally omitted —
        it causes disconnect storms in weak-coverage areas where clients get
        kicked and can't find a better AP.
        """
